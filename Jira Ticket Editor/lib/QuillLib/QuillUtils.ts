...
async function uploadImage(issueID: string, file: File | undefined, quillRef: React.RefObject<ReactQuill | null>) {
  ...
  if (response?.status.toString().startsWith("2")) {
    ...
    attachments = (await response.json()) as AttachmentInterface[];

    if (attachments.length > 0) {

      // Construct Image URL
      imageURL = new URL("/proxy-api", window.location.origin);
      imageURL.searchParams.append("pathname", `/attachment/content/${attachments[0].id}`);
  ...
  if (imageURL) {
    ...
    const image: HTMLImageElement | null = await loadImageAsync(imageURL.toString());
    ...
    if (response?.status.toString().startsWith("2")) {
      const sourceURL: string | null = response.headers.get("origin-location");
